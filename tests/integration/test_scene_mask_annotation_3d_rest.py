"""
Integration tests — REST endpoint POST /annotation/scene-mask-annotation-3d.
"""
...
ENDPOINT = "/annotation/scene-mask-annotation-3d"
...
def test_rest_default_model(
    ...
    run_dir = tmp_path / "rest_scene_default"
    timings = []
    for run in range(1, N_RUNS + 1):
        ...
        response = client.post(
            ...
            data={"model_backend": local_scene_model_path, "return_annotated_cloud": "true"},
        )
        ...
        for ann in data["annotations"]:
            ...
            assert ann["pixel_count"] > 0
            assert base64.b64decode(ann["mask_image"])[:4] == b"\x89PNG"
    ...
    for i, t in enumerate(timings[1:], start=2):
        assert t < WARM_LIMIT, f"[REST/Scene] Run {i}: {t:.3f}s > {WARM_LIMIT}s"


def test_rest_custom_inference_size(
    indoor_image_bytes, indoor_point_cloud_ply, local_scene_model_path
):
    SceneMaskAnnotation3DHandler._scene_models.clear()
    resp = client.post(
        ENDPOINT,
        ...
        data={"model_backend": local_scene_model_path, "inference_size": "384"},
    )
    assert resp.status_code == 200
    assert "annotations" in resp.json()


def test_rest_region_clouds_returned(
    indoor_image_bytes, indoor_point_cloud_ply, local_scene_model_path
):
    SceneMaskAnnotation3DHandler._scene_models.clear()
    resp = client.post(
        ...
        data={"model_backend": local_scene_model_path, "return_region_clouds": "true"},
    )
    assert resp.status_code == 200
    for ann in resp.json()["annotations"]:
        if ann["point_indices"]:
            assert ann["region_cloud_ply"] is not None
            assert base64.b64decode(ann["region_cloud_ply"]).startswith(b"ply\n")


def test_rest_no_image_uses_front_view(indoor_point_cloud_ply, local_scene_model_path):
    ...
    resp = client.post(
        ...
        data={"model_backend": local_scene_model_path},
    )
    assert resp.status_code == 200
    assert "annotations" in resp.json()
